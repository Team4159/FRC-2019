...
public class CollisionAvoidance {
    ...
    public static boolean raiserSafeToBeUp(int elevator_position, int elevator_goal) {
         ...
         */
        return elevator_position < Constants.POSITIONS.FEEDER_DANGEROUS_LOWER_BAND && elevator_goal < Constants.POSITIONS.FEEDER_DANGEROUS_LOWER_BAND;
    }
    ...
    public static boolean safeToMoveElevator(int elevator_position, int elevator_goal, int feeder_position, boolean raiser_extended) {
        boolean feeder_up = feeder_position >= (Constants.POSITIONS.FEEDER_STOWED + 100);
    ...
    private static boolean elevatorNotMovingUpThroughFeeder(int elevator_position, int elevator_goal) {
        return elevator_position < Constants.POSITIONS.FEEDER_DANGEROUS_LOWER_BAND
                && elevator_goal < Constants.POSITIONS.FEEDER_DANGEROUS_LOWER_BAND;
    }
    ...
    private static boolean elevatorNotMovingDownThroughFeeder(int elevator_position, int elevator_goal) {
        return elevator_goal > Constants.POSITIONS.FEEDER_DANGEROUS_HIGHER_BAND
                && elevator_position > Constants.POSITIONS.FEEDER_DANGEROUS_HIGHER_BAND;
    }
    ...
    private static boolean elevatorNotMovingUpThroughRaiser(int elevator_position, int elevator_goal) {
        return elevator_position < Constants.POSITIONS.RAISER_DANGEROUS_LOWER_BAND
                && elevator_goal < Constants.POSITIONS.RAISER_DANGEROUS_LOWER_BAND;
    }

    // Returns whether or not the elevator is moving down through the feeder
    private static boolean elevatorNotMovingDownThroughRaiser(int elevator_position, int elevator_goal) {
        return elevator_goal > Constants.POSITIONS.RAISER_DANGEROUS_HIGHER_BAND
                && elevator_position > Constants.POSITIONS.RAISER_DANGEROUS_HIGHER_BAND;
    }
}
